from flask import Flask, url_for, render_template, request, redirect, session, request, abort
...
def get_state(lat, lng):
    response = requests.get(f'https://maps.googleapis.com/maps/api/geocode/json?latlng={lat},{lng}&key=AIzaSyAhGURYDl089JjSC-xBtgfjQ3QhP8UNg9k')
    data = response.json()
        ...
        if request.method == 'POST':
            ...
            try:
                if reference is not None and reference[:3] == 'QA-':
                    ...
                    search  = reference_number.query.filter_by(number=f'{new_reference}').first()
...
def register():
    if request.method == 'POST':
        ...
        if login_info.query.filter_by(username=request.form['username']).first() == None:
            new_user = login_info(username=request.form['username'], password=request.form['password'], ip_address=ip_address, sus=False)
...
def call():
    ...
    first_number = reference_number.query.order_by(reference_number.number.desc()).first()
    print(first_number)
    try:
    ...
    if sus_user.sus == True:
        ...
        for location in similar_locations:
            print(f"Similar location found: Longitude={location.longitude}, Latitude={location.latitude}, User={location.user}")
...
def responders_login():
    ...
    if request.method == 'GET':
        ...
        passw = request.form['password']
        try:
            data = responders_login_info.query.filter_by(username=names, password=passw).first()
